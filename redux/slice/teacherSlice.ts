...
import { TViewAllState } from "@/types/teacher";
  ...
  async (_, thunkApi) => {
    try {
      const response = await axiosClient.get(
        "/mobile/insights/teacher-insights"
      );
...
export const JoinedCheckStatus = createAsyncThunk(
  ...
  async (groupId: string, thunkApi) => {
    try {
      const response = await axiosClient.get(
        `/mobile/groups/joined-or-not/${groupId}`
      );
      return response.data;
    } catch (error: unknown) {
      if (error instanceof AxiosError) {
...
export const GroupJoinStudents = createAsyncThunk(
  "teacher/group-join-students",
  async (_, thunkApi) => {
    try {
      const response = await axiosClient.get("/mobile/groups/view-students");
      return response.data;
    } catch (error: unknown) {
      if (error instanceof AxiosError) {
        return thunkApi.rejectWithValue(
          error.response?.data?.detail ||
            "Fetching group joined students failed"
        );
      }
    }
  }
  ...
  async (
    ...
    try {
      const response = await axiosClient.put(
        `/mobile/notes/edit-note/${data.noteId}`,
        {
          title: data.title,
