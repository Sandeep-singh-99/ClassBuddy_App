...
export const DocsFetch = createAsyncThunk("docs/fetch", async (_, thunkApi) => {
  try {
    const response = await axiosClient.get("/mobile/docs/my-docs");
    return response.data;
  } catch (error) {
    if (error instanceof AxiosError) {
      return thunkApi.rejectWithValue(
...
export const DocsFetchById = createAsyncThunk(
  ...
  async (docId: string, thunkApi) => {
    try {
      const response = await axiosClient.get(`/mobile/docs/my-docs/${docId}`);
      return response.data;
    } catch (error) {
      if (error instanceof AxiosError) {
        return thunkApi.rejectWithValue(
  ...
  async (_, thunkApi) => {
    try {
      const response = await axiosClient.get(
        "/mobile/docs/teacher-notes-with-docs"
      );
      return response.data;
    } catch (error) {
      if (error instanceof AxiosError) {
